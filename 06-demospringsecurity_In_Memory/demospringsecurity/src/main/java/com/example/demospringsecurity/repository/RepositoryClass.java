...
public class RepositoryClass {
    ...
    /**
     * the repository file contains the code of InMemoryDB related stuffs  and data storage
     * here InMemoryUserDetailsManager is solely responsible for storing the harcoded users
     * also including the user from outside into it per app session
     *
     *      you need to make sure that the password present in inmemoryDB should be in this format as we are creating
     *      bean of the BCryptPasswordEncoder so it excepts data from DB in Bcrypted format
     *      if not used you cas use noops instance then , in raw format you could have stored the passwd ( risky )
     *
